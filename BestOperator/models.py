    ...
    def __str__(self):
        ...
        if self.order_from_date:
            ...
            if self.order_to_date:
                ...
                res_str += _n(' from %s') % (self.order_from_date)
        return res_str
    def get_name(self):
        return self.__str__()
    class Meta:
...
class Payment(NameDescrAutoGeneratedModel):
    ...
    def __str__(self):
        ...
        if self.feature:
            res_str += _n(" (%(feature)s)") % {'feature' : self.feature}
        if self.period:
...
class ServiceType(NameDescrAutoGeneratedModel):
    is_displayed = models.BooleanField(default=False, verbose_name=_("Is Displayed on Main Form"), help_text=_("Specifies whether to display this service type on the user-inout form or not."))
    def __str__(self):
        return self.name
    def get_name(self):
        return self.name
...
class Direction(NameDescrAutoGeneratedModel):
    ...
    to_operator = models.ForeignKey('Operator', verbose_name=_("To Operator"), null=True, blank=True,
                                    help_text=_("Destination operator, who will receive call or message"))
    def __str__(self):
        res_str = "%(from_loc)s" % {'from_loc' : self.from_location}
        if (self.to_location):
            res_str += _n(" -> %(to_loc)s") % {'to_loc' : self.to_location}
        if (self.to_operator):
            res_str += _n(" (%(to_oper)s)") % {'to_oper' : self.to_operator}
        return res_str
    def get_name(self):
        return self.__str__()
    class Meta:
...
class Param(NameDescrAutoGeneratedModel):
    ...
    def __str__(self):
        ...
        if self.value:
            res_str += _n(" = %(val)s") % {'val' : self.value}
        return res_str
    def get_name(self):
